...
class Downloader extends React.Component {
  ...
  buttonsToggle = () => {
    ...
    const toSet = !open ? { open: true, forOpen: true } : { open: false };
    this.setState(toSet);
    if (open) {
      setTimeout(() => {
  ...
  download = (withText = true) => () => {
    ...
    this.timeout = setTimeout(() => {
      ...
      this.props.addAlert(
        `There was an error while rendering the image. Please retry.`
      );
    }, 5000);
  ...
  withText = () => {
    ...
    html2canvas(this.textViewNode, options)
      ...
      .catch((_) => {
        ...
        this.props.setLoading(false);
        this.props.addAlert(
          `There was an error while rendering the text. Please retry.`
        );
      });
  };
  deliver = (canvas) => {
  ...
  render() {
    ...
    const textView = !this.state.renderTextView ? null : (
      <div
        className={classes.textView}
        ref={(ref) => {
          this.textViewNode = ref;
        }}
      >
        <TextView
    ...
    return (
      <div className={className}>
        ...
        >
          ...
          <Grow in={!open}>
            <Button
              ...
              classes={{
                root: classnames(classes.button, classes.saveButton)
              }}
              aria-label="save"
              raised
              fab
        ...
        <div className={classes.parent}>{textView}</div>
      </div>
    );
  }
}

export default compose(withStyles(styles), connector)(Downloader);
