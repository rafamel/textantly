...
function maxDrawn() {
  ...
  this.data.maxDrawn = {
    width: source.width * canvasData.naturalWidth / imageData.naturalWidth,
    height: source.height * canvasData.naturalHeight / imageData.naturalHeight
  };
}
...
function canvas() {
  ...
  canvas.position = {
    x: (offset.left + canvasData.width / 2) / containerData.width,
    y: (offset.top + canvasData.height / 2) / containerData.height
  };

  canvas.visible = {
    width: {
      start: offset.left < 0 ? -(offset.left / canvasData.width) : 0,
      end: offset.right < 0 ? 1 + offset.right / canvasData.width : 1
    },
    height: {
      start: offset.top < 0 ? -(offset.top / canvasData.height) : 0,
      end: offset.bottom < 0 ? 1 + offset.bottom / canvasData.height : 1
    }
  };
...
function crop() {
  ...
  const approximate = (approximateN, value) => {
    if (!value) return 0;
    return Math.abs(approximateN - value) <= A ? approximateN : value;
  };
