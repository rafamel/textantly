...
const viewMode = selectorWithType({
  ...
  select: [(state) => state.edits.navigation.image],
    ...
    return (
      <div
        className={classnames({
          ...
          [classes.noCropBox]:
            this.state.noCropBox || (viewMode && !this.state.activeCrop),
          [classes.viewMode]: viewMode,
          [classes.hiddenViewMode]:
            this.state.hidden && viewMode && this.state.activeCrop
        })}
      >
        <Cropper
          ref={(ref) => {
...
export default compose(withStyles(styles), broadcaster, connector)(ImageView);
