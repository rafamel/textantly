...
class TextView extends React.Component {
  ...
  loadFont = (fontFamily, previousFF, keepOpacity) => {
    ...
    fontLoad(fontFamily)
      ...
      .catch(() => {
        ...
        this.props.addAlert(
          `Font could not be loaded. Do you have an active internet connection?`
        );
      });
  };
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.loadFont(
      nextProps.textEdits.fontFamily,
      this.props.textEdits.fontFamily
    );
    this.fontResize();
  }
  componentDidUpdate() {
  ...
  render() {
    ...
    const textString =
      this.props.textEdits.textString || config.defaults.text.textString;
    ...
    const line =
      ...
      drawnDimensions.width * overlayWidth * 0.01 < 175 ? null : (
        <span
    ...
    return (
      <div style={style} className={classnames(classes.root, classes.text)}>
        <div className={classnames(classes.overlay, classes.overlayPosition)}>
          {line}
          {line}
          <div
            className={classnames(classes.contain, classes.containPosition)}
            style={{ opacity }}
          >
            <TextResizer
              text={textString}
