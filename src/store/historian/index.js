...
function addCan(history) {
  ...
  const can = {
    ...
    forwards: temp ? false : helper(arr, i - 1, +1)
  };
...
export default function history({ key, exclude }) {
  ...
  function insert(previous, updated) {
    ...
    const arr = index === -1 ? history.arr : history.arr.slice(0, index);

    // Leave only the last non skippable onNext
    const lastNonSkippable = history.onNextArr.reduce(
      (acc, x, i) => (!x.skip ? i : acc),
      null
    );
    const onNextArr =
      lastNonSkippable == null
        ? history.onNextArr
  ...
  function insertNext(previous, updated, skip = false) {
    ...
    return {
      ...
      [key]: {
        ...previous[key],
        onNextArr: previous[key].onNextArr.concat([
          { diff: diffObj, onNext: true, skip }
        ])
      }
    };
  }
  ...
  function backwards(current) {
    ...
    const select =
      history.index === -1 ? history.arr.length - 1 : history.index - 1;
    ...
    return selected.skip ? backwards(updated) : updated;
  }
  ...
  function forwards(current) {
    ...
    return selected.skip ? forwards(updated) : updated;
  }
